...
/// For advanced controls, tap the body to open the full screen.
...
class _MiniPlayerBody extends StatelessWidget {
  ...
  ({int? partNumber, int totalParts}) _resolveNeighbor(
      BuildContext context, int offset) {
    final state = AppState.of(context);
    ...
    if (bookId == null ||
        teacherId == null ||
        partNumber == null) {
      return (partNumber: null, totalParts: 0);
    }
    try {
  ...
  Future<void> _switchTo(
    ...
    final bookId = state.audioService.currentBookId;
    final teacherId = state.audioService.currentTeacherId;
    if (bookId == null || teacherId == null) return;
    ...
    try {
      ...
      final path =
          await state.downloadService.localPath(fileId);
      if (path == null) return;

      await state.audioService.stop();

      final lessonTitle =
          ArabicUtils.lessonTitle(newPartNumber);
      final coverPath =
